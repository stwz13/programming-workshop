...
void test_allocator_initialization() {
  ...
  assert(linear_allocator_init(NULL, buffer, TEST_BUFFER_SIZE) ==
         LINEAR_ALLOC_INVALID_ARG);
  assert(linear_allocator_init(&allocator, NULL, TEST_BUFFER_SIZE) ==
         LINEAR_ALLOC_INVALID_ARG);
  assert(linear_allocator_init(&allocator, buffer, 0) ==
         LINEAR_ALLOC_INVALID_ARG);
  assert(linear_allocator_init(&allocator, buffer, TEST_BUFFER_SIZE) ==
